...
const BannerCard = (props: Props) => {
  const { index } = props
  return (
    <Link to={`/manga/${'sono-bisque-doll-wa-koi-wo-suruu'}?banner=${index}`}>
      <Card className="group relative flex cursor-pointer items-center overflow-hidden p-0 before:absolute before:left-0 before:top-0 before:block before:h-full before:w-1 before:bg-red-1">
        <div className="absolute right-2 top-2 z-10 flex h-8 w-8 items-center justify-center rounded-full border-2 border-red-1 bg-dark-6 opacity-0 shadow-lg transition-all duration-150 group-hover:opacity-100">
        ...
        <div className="relative z-10 w-2/3 p-4 pr-8">
          ...
          <Text className="mt-8 !line-clamp-2 hidden leading-4 text-light-grey-3 md:block">
          ...
          <Text className="mt-2 text-light-grey-1">Chap 100 - Vol 24</Text>
